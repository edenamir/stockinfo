  ...
  if (window.location.href.includes("signin")) {
    ...
    for (var i = 0; i < cookies.length; i++) {
      ...
      if (cookie.startsWith("loginInfo=")) {
        var emailValue = decodeURIComponent(
          cookie.substring("loginInfo=".length)
        );
      ...
      if (cookie.startsWith("password=")) {
        var passwordValue = decodeURIComponent(
          cookie.substring("password=".length)
        );
  ...
  if (isSignUp == true) {
    ...
    if (rememberMeChecked) {
      ...
      document.cookie = `loginInfo=${encodeURIComponent(
        emailValue
      )}; expires=Thu, 1 Jan 2030 00:00:00 UTC; path=/`;
...
function sendPost(pageType) {
  ...
  if (pageType == "signup") {
    document.getElementById("registration-form-signup").addEventListener(
      "submit",
      (event) => {
        event.preventDefault();
  ...
  if (pageType == "contactus") {
    document.getElementById("registration-form-contactus").addEventListener(
      "submit",
      (event) => {
        event.preventDefault();
